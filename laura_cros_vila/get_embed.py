"""
    ...
    Example:
    # python get_embed.py -m clap-laion-music -d /data/suno/audio /data/udio/audio
"""
...
import os
import json
from model_loader import CLAPMusic, MusiCNN
    ...
    for model_name in args.models:
        ...
        for d in args.dirs:
            ...
            for mp3 in glob.glob(d + '/*.mp3'):
                npy_path =  Path(d) / 'embeddings' / model.name / (Path(mp3).stem + '.npy')
                import hashlib

                short_name = hashlib.md5(Path(mp3).stem.encode()).hexdigest() + '.npy'
                npy_path = Path(d) / 'embeddings' / model.name / short_name
